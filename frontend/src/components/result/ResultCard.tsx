...
export function ResultCard({ result, onReset }: ResultCardProps) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-24 duration-1000">
        ...
        {/* Background Glows */}
        <div className="absolute -top-40 -right-40 w-96 h-96 bg-brand-primary/20 blur-[150px] pointer-events-none animate-pulse-slow"></div>
        ...
        <div className="bg-[#0a0a0b]/80 rounded-[1.8rem] overflow-hidden relative z-10">
            ...
            <header className="p-10 md:p-14 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 relative overflow-hidden">
                ...
                <div className="space-y-4 relative z-10">
                    <div className="inline-flex items-center gap-3 px-3 py-1 rounded-full bg-brand-primary/10 border border-brand-primary/20 text-brand-primary text-[10px] font-black uppercase tracking-[0.2em]">
                ...
                <button 
                    ...
                    className="px-8 py-4 bg-brand-primary hover:bg-[#ff904d] text-black font-black uppercase tracking-[0.2em] text-xs rounded-full shadow-[0_0_30px_-5px_rgba(0,255,194,0.4)] hover:shadow-[0_0_50px_-10px_rgba(0,255,194,0.6)] hover:scale-105 transition-all active:scale-95 flex items-center gap-3 cursor-pointer"
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2">
                ...
                <div className="p-10 md:p-14 border-b lg:border-b-0 lg:border-r border-white/5 space-y-12">
                    ...
                    <div className="space-y-8">
                        ...
                        {result.parallel_logic && <span className="text-[10px] font-mono text-brand-primary bg-brand-primary/10 px-2 py-1 rounded">PARALLEL</span>}
                        </div>
                        ...
                        {result.devices.map((dev, i) => (
                            ...
                            {i !== result.devices.length - 1 && (
                                <div className="absolute left-[19px] top-10 w-0.5 h-6 bg-white/10 group-hover:bg-brand-primary/50 transition-colors"></div>
                            )}
                            <div className="w-10 h-10 rounded-xl bg-bg-deep border border-white/10 flex items-center justify-center text-text-dim font-black text-sm group-hover:border-brand-primary group-hover:text-brand-primary group-hover:shadow-[0_0_15px_rgba(0,255,194,0.2)] transition-all z-10">
                                {i + 1}
                            </div>
