...
import { History, Zap, CreditCard, Library } from "lucide-react";
  ...
  return (
    <aside className="w-64 h-screen fixed left-0 top-0 border-r border-white/5 bg-bg-surface/30 backdrop-blur-xl flex flex-col z-50">
            ...
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group
                ${isActive || isHomeActive
                  ? "bg-brand-primary/10 text-brand-primary shadow-[0_0_20px_-5px_rgba(0,255,194,0.3)]" 
                  : "text-text-dim hover:text-white hover:bg-white/5"
                }`}
            >
              <item.icon className={`w-5 h-5 ${isActive ? "text-brand-primary" : "text-text-dim group-hover:text-white"}`} />
              <span className="font-medium text-sm tracking-wide">{item.label}</span>
              
              {isActive && (
                <div className="ml-auto w-1.5 h-1.5 rounded-full bg-brand-primary shadow-[0_0_8px_rgba(0,255,194,0.8)]" />
              )}
            </Link>
          );
        })}
      ...
      <div className="p-4 m-4 rounded-2xl bg-[#0a0a0b] border border-white/5 flex items-center gap-4 relative overflow-hidden group">
        {/* Glow Effect */}
        <div className="absolute -bottom-10 -right-10 w-24 h-24 bg-brand-primary/5 blur-3xl rounded-full group-hover:bg-brand-primary/10 transition-colors" />
        ...
        <div className="flex flex-col z-10">
            ...
            <div className="flex items-baseline gap-1.5">
                ...
                <span className="text-[10px] text-brand-primary font-bold uppercase tracking-wider">Credits</span>
            </div>
        </div>
      </div>
    </aside>
