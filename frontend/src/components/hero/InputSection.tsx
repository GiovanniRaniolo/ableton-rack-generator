...
export function InputSection({ prompt, setPrompt, handleGenerate, isGenerating, error }: InputSectionProps) {
  return (
      <section className="space-y-12 text-center animate-in fade-in zoom-in duration-700 flex flex-col items-center">
        <div className="space-y-6 flex flex-col items-center">
           <Logo className="w-24 h-24 mb-4" showText={false} />
              ...
              <div className="flex justify-between items-center px-4 pb-2">
                 <div className="flex gap-2">
                    {TEMPLATES.slice(0,3).map(t => (
                      <button key={t.id} onClick={() => setPrompt(t.prompt)} className="px-3 py-1.5 rounded-full bg-white/5 hover:bg-white/10 text-[10px] font-bold uppercase tracking-widest transition-colors border border-white/5 flex items-center gap-2 text-text-bright hover:text-white">
                        {t.icon}
                        {t.name}
                       </button>
                    ))}
